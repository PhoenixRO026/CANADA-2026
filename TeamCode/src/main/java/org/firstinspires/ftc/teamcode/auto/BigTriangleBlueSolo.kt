...
class BigTriangleBlueSolo : LinearOpMode() {
    ...
    private val startPose = Pose(144 - 123.0, 118.0, Math.toRadians(144.0))
    ...
    private fun buildPaths() {
        ...
        intakeMiddle = robot.follower.pathBuilder()
            .addPath(BezierCurve(bigTriangleShootPose, Pose(144 - 83.5, 58.0), intakeMiddlePose))
            .setConstantHeadingInterpolation(Math.toRadians(180.0 - 0.0))
            .build()

        shootMiddle = robot.follower.pathBuilder()
            .addPath(BezierCurve(intakeMiddlePose, Pose(144 - 106.5, 60.0), bigTriangleShootPose))
            .setConstantHeadingInterpolation(Math.toRadians(180.0 - 0.0))
            .setTranslationalConstraint(0.07)
            .build()

        openGate = robot.follower.pathBuilder()
            .addPath(BezierCurve(bigTriangleShootPose, Pose(144 - 106.5, 65.0), gateApproachPose))
            .setConstantHeadingInterpolation(Math.toRadians(180.0 - 0.0))
            .build()
...
//            .setTranslationalConstraint(2.0)
        ...
        turnGate = robot.follower.pathBuilder()
            .addPath(BezierCurve(gateRamPose, Pose(144 - 125.0, 53.0), turnToWall))
...
//            .setConstantHeadingInterpolation(gateRamPose.heading)
        ...
        shootGate = robot.follower.pathBuilder()
            .addPath(BezierCurve(turnToWall, Pose(144 - 104.5, 50.0), bigTriangleShootPose))
            .setConstantHeadingInterpolation(Math.toRadians(180.0 - 0.0))
            .setTranslationalConstraint(0.07)
            .build()

        intakeClose = robot.follower.pathBuilder()
            .addPath(BezierCurve(bigTriangleShootPose, Pose(144 - 91.5, 83.0), intakeClosePose))
            .setConstantHeadingInterpolation(Math.toRadians(180.0 - 0.0))
            .build()
